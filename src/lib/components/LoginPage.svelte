<script lang="ts">
    ...
    import { User, Lock, AlertCircle, Loader2, ArrowLeft } from 'lucide-svelte';
...
<div class="flex min-h-[100dvh] items-center justify-center bg-chrome-900 px-4 py-10">
    <div class="w-full max-w-sm">
        ...
        <div class="rounded-2xl border border-chrome-700 bg-chrome-800 p-6 shadow-raised sm:p-8">
            <h1 class="text-lg font-semibold text-chrome-100">Login</h1>
            <p class="mt-1 text-sm text-chrome-400">Sign in to continue to your circuits.</p>

            <form class="mt-6 space-y-4" onsubmit={handleLogin}>
                <div>
                    <label for="username" class="mb-1.5 block text-xs font-medium uppercase tracking-wide text-chrome-400">
                        Username
                    </label>
                    <div class="relative">
                ...
                <div>
                    <label for="password" class="mb-1.5 block text-xs font-medium uppercase tracking-wide text-chrome-400">
                        Password
                    </label>
                    <div class="relative">
            ...
            <p class="mt-5 text-center text-sm text-chrome-400">
                ...
                <button type="button" class="font-medium text-accent hover:underline" onclick={() => (location = '/register')}>
                    Sign up
                </button>
            </p>
        </div>
